...
public class Skill5 : MonoBehaviour, IPointerDownHandler, IPointerUpHandler
{
    ...
    void Start()
    {
        ...
        Transform t = player.transform.Find("FifthSkillEffect");
        if (t != null)
        {
            ParticleSystem[] ps = t.GetComponentsInChildren<ParticleSystem>(true);
            m_particles.AddRange(ps);
        }
        GameObject b = GameObject.Find("UI/AttackButton/Skill5");
        if (b != null)
        {
        ...
        maxCoolTime = Database.instance.skills.Find(ex => ex.name == "Skill5").coolTime;
        coolTime = maxCoolTime;
        Transform r = player.transform.Find("FifthSkillRange");
        if (r != null)
            range = r.gameObject;
